...
Use the PATH command `mvp-loop-supervised` as the only coding execution path. Frontend Agents only start the workflow; local Qwen may code low/medium-risk work, while cloud Codex performs independent review and supervised takeover.
